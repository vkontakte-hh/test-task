...
def request():
    url = 'https://{host}:8443/?database={db}&query={query}'.format(
        ...
        db='vkontakte',
        query='CREATE TABLE vkontakte.t_pare_table(pare_id UInt8, Pare_one String, Pare_two String, Pare_three String) ENGINE = MergeTree() PARTITION BY pare_id ORDER BY (pare_id)'
    )
    auth = {
